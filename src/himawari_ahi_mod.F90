...
!        - Convert Himawari NetCDF files to ioda-v1 format.
 !     2. Get Himawari L2 products of cloud mask (CMSK), cloud height (CHGT) and cloud phase (CPHS) NetCDF data and add it to the data structure
 !
! Caveats:
...
! TODO: implement get brigthness temperature directly from NetCDF files (from ftp.ptree.jaxa.jp)
  ...
  integer(i_kind),    allocatable :: segm(:)
  ...
  logical,            allocatable :: is_HS_HSD(:), is_HS_NC(:)
  
  character(len=256), allocatable :: hsfnames(:)
  ...
  integer(i_kind), allocatable :: ctph_2d(:,:)   ! cloud phase
  real(r_kind),    allocatable :: ctt_2d(:,:) !cloud top temperature  
...
subroutine Himawari_ReBroadcast_converter(glon_out, glat_out, F_out, varname_out, got_latlon_out)
   ...
   allocate (is_HS_HSD(nfile))
   allocate (is_HS_NC(nfile))
   ...
   call decode_himawari_name(trim(fnames(ifile)), finfo, fband_id(ifile), segm(ifile), fsat_id, scan_time(ifile), julianday(ifile), is_CLP(ifile), is_BCM(ifile), is_Phase(ifile), is_HT(ifile), is_HS_HSD(ifile), is_HS_NC(ifile))
     ...
     if ( .not. ( is_CLP(ifile) .or. is_BCM(ifile) .or. is_Phase(ifile) &
        ...
        if ( finfo /= data_id ) then
           write(0,*) 'Data ID from namelist /= Data ID in flist.txt'
           cycle file_loop1
        else
           ! only process band 7-16
           if ( fband_id(ifile) == -99 ) then ! means that all bands are in one file
              continue
           else if ( fband_id(ifile) < band_start .or. fband_id(ifile) > band_end ) then
   ...
   if ( any(valid) ) then
      if ( got_hs ) then
         ...
         if ( got_cm .and. got_phase .and. got_ht ) then
 
            write(0,*) 'allocate (F_out(npixel, nline, nband+3))'
            allocate (F_out(npixel, nline, nband+3)) 
            allocate (varname_out(nband+3))   
            F_out(:,:,:)   = missing_r
            varname_out(:) = ''		 
         else
     ...
     if ( got_clp ) then
            ...
            if ( .not. got_nc .and. ( nx /= npixel .or. ny /= nline ) ) then
                write(0,*) 'ERROR: Brightness temperature and cloud '&
                       'type are at different resolutions. Try another product'
                stop
            end if			
	  ...
	  if ( got_cm ) then
	   ...
	   if ( got_ht ) then
            write(0,*) 'Added: reading AHI L2 CloudTopPress files'
            ...
            if ( nx /= npixel .or. ny /= nline ) then
               write(0,*) 'ERROR: Brightness temperature and cloud '&
                          'height are at different resolutions. Try another product'
               stop
            end if			
            ...
            if ( .not. allocated(rdata(it)%cm) )  allocate (rdata(it)%cm(nx,ny))
            rdata(it)%cm(:,:) = ctt_2d(:,:)
            F_out(:,:,nband+2) = ctt_2d(:,:)
            varname_out(nband+2) = 'TEMP_'//fsat_id

        end if  
		if ( got_phase ) then
            write(0,*) 'Added: reading AHI L2 CloudPhase files'
            ...
            if ( nx /= npixel .or. ny /= nline ) then
               write(0,*) 'ERROR: Brightness temperature and cloud '&
                          'Phase are at different resolutions. Try another product'
               stop
            end if				
   ...
   if ( allocated(ctt_2d) )  deallocate(ctt_2d)
   if ( allocated(ctph_2d) )  deallocate(ctph_2d)   
   ! write IODAv1 file
...
subroutine read_CLP(ncid, nx, ny, cm)
   ...
   integer(i_short),  allocatable   :: uitmp_short_2d(:,:)
   integer(i_kind),   allocatable   :: itmp_2d(:,:)
   integer(i_kind),   allocatable   :: uitmp_2d(:,:)
   integer(i_kind)                  :: nf_status
   integer(i_kind)                  :: istart(2), icount(2)
   integer(i_kind)                  :: varid, i, j
   integer(i_kind)                  :: imiss = -999
   integer(i_short)                 :: cifill = -999
   integer(i_short)                 :: ifill = -999
   real(r_single)                   :: scalef, offset

   character(len=100)               :: flag_description
   character(len=256),  allocatable :: flags(:,:)
   integer :: clear_flag, cloudy_flag
   ...
   do j = 1, ny
      do i = 1, nx
         if (itmp_short_2d(i,j) /= cifill) then
            itmp_2d(i,j) = offset + itmp_short_2d(i,j) * scalef
         end if
      end do
   end do

   ! read QA (Quality Assurance)
   nf_status = nf_INQ_VARID(ncid, 'QA', varid)
   allocate(uitmp_short_2d(nx,ny))
   if (nf_status /= NF_NOERR) then
      print *, "Error: Unable to find the variable 'QA' in NetCDF file"
      deallocate(uitmp_short_2d)
      return
   end if
   nf_status = nf_GET_VARA_INT2(ncid, varid, istart(1:2), icount(1:2), uitmp_short_2d(:,:))
   nf_status = nf_GET_ATT_INT2(ncid, varid, 'missing_value',  ifill)
   nf_status = nf_GET_ATT_REAL(ncid, varid, 'scale_factor', scalef)
   nf_status = nf_GET_ATT_REAL(ncid, varid, 'add_offset', offset)

   allocate(uitmp_2d(nx, ny))
   uitmp_2d = uitmp_short_2d
   do j = 1, ny
      do i = 1, nx
         if (uitmp_short_2d(i,j) /= ifill) then
            if ( uitmp_short_2d(i,j) .lt. 0_i_short ) then
               uitmp_2d(i,j) = uitmp_2d(i,j) + 65536
            end if
            uitmp_2d(i,j) = offset + uitmp_2d(i,j) * scalef
         end if
      end do
   end do
   ! uitmp_2d is now a 32-bit integer

   allocate(flags(nx, ny))
   do j = 1, ny
      do i = 1, nx
         if (uitmp_2d(i,j) /= ifill) then
            call interpret_qa_flags(uitmp_2d(i,j), clear_flag, cloudy_flag)

            if ( clear_flag == 1 .and. itmp_2d(i,j) == 0 ) then
               cm(i,j) = 0  ! Confidently clear
            elseif (cloudy_flag == 1 .and. itmp_2d(i,j) /= 0 .and. itmp_2d(i,j) /= cifill) then
               cm(i,j) = 1  ! Confidently cloudy
            end if
         end if
      end do
   end do
...
subroutine read_L2_TEMP(ncid, nx, ny, ctt, time_start)
   ...
   integer(i_byte),  allocatable    :: itmp_byte_2d(:,:)
   integer(i_short), allocatable    :: itmp_short_2d(:,:)
   real(r_kind),  allocatable    :: itmp_2d(:,:)
   integer(i_kind)                  :: nf_status
   integer(i_kind)                  :: istart(2), icount(2)
   integer(i_kind)                  :: varid, i, j
   ...
   nf_status = nf_GET_VARA_REAL(ncid, varid, istart(1:2), icount(1:2), itmp_2d(:,:))
   nf_status = nf_GET_ATT_REAL(ncid, varid, '_FillValue',  ifill)


   ctt(:,:) = rmiss
   do j = 1, ny
      do i = 1, nx
          if (qf_temp(i,j) == 0 ) then ! good quality
              ctt(i,j) = itmp_2d(i,j)
          end if
      end do
   end do
   write(*,*) "min/max of cldTopPres =", minval(ctt), maxval(ctt)
...
subroutine read_L2_Phase(ncid, nx, ny, ctph, time_start)
   ...
   integer(i_kind),   intent(in)    :: ncid
   integer(i_kind),   intent(in)    :: nx, ny
   integer(i_kind),   intent(inout) :: ctph(nx,ny)
   character(len=22), intent(out)   :: time_start  ! 2017-10-01T18:02:19.6Z
   integer(i_byte),  allocatable    :: itmp_byte_2d(:,:)
   integer(i_kind)                  :: nf_status
   integer(i_kind)                  :: istart(2), icount(2)
   ...
   icount(2) = ny
   allocate(itmp_byte_2d(nx,ny))
   nf_status = nf_INQ_VARID(ncid, 'CloudType', varid) ! 1-8 type
   nf_status = nf_GET_VARA_INT1(ncid, varid, istart(1:2), icount(1:2), itmp_byte_2d(:,:))
   ctph(:,:) = rmiss
   do j = 1, ny
      do i = 1, nx
         !if ( qf_type(i,j) == 0 ) then ! good quality
...
subroutine read_HS_NC(ncid, nx, ny, nband, bt, gsolzen, gsatzen, time_start)
   ...
   real(r_kind),      intent(inout) :: bt(nx,ny,nband)
   real(r_kind),      intent(inout) :: gsolzen(nx,ny)
   real(r_kind),      intent(inout) :: gsatzen(nx,ny)
   character(len=22), intent(out)   :: time_start  ! 2017-10-01T18:02:19.6Z
   ...
   integer(i_kind)                  :: varid, i, j
   integer(i_short)                 :: ifill = -999
   real(r_single)                   :: rfill_lat, rfill_lon
   real(r_single),  allocatable     :: geometry_parameters(:)
   real(r_kind)                     :: rmiss = -999.0
   integer(i_kind)                  :: imiss = -999
   real(r_single)                   :: scalef, offset

   real(r_double)   :: lon_sat, h_sat, r_eq, Rs
   integer          :: geometry, dimsize, iband
   character(len=2) :: siband

   scalef = 0.01
   offset = 0.0

   nf_status = nf_GET_ATT_TEXT(ncid, nf_GLOBAL, 'start_time', time_start)

   ! get geometry parameters
   nf_status = nf_INQ_VARID(ncid, 'geometry_parameters', varid)
   nf_status = nf_INQ_VARDIMID(ncid, varid, geometry)
   nf_status = nf_INQ_DIMLEN(ncid, geometry, dimsize)

   allocate(geometry_parameters(dimsize))
   nf_status = nf_GET_VAR_REAL(ncid, varid, geometry_parameters)

   lon_sat = geometry_parameters(1) * deg2rad
   r_eq = geometry_parameters(7)
   !Rs  : distance from Earth center to satellite= r_eq + satDis (h_sat)
   Rs = geometry_parameters(6)
   h_sat = Rs - r_eq
   ...
   icount(2) = ny
   bt(:,:,:)  = rmiss
   ! get brightness temperature for each band
   do iband = 1, nband
      write(siband, '(i2.2)') iband
      nf_status = nf_INQ_VARID(ncid, 'tbb_'//siband, varid)
      if (nf_status /= NF_NOERR) then
        cycle  ! Skip this band
      end if
      allocate(itmp_short_2d(nx, ny))
      nf_status = nf_GET_VARA_INT2(ncid, varid, istart(1:2), icount(1:2), itmp_short_2d(:,:))
      nf_status = nf_GET_ATT_INT2(ncid, varid, 'missing_value', ifill)
      ...
      do j = 1, ny
         do i = 1, nx
            if ( itmp_short_2d(i,j) /= ifill ) then
               bt(i,j,iband) = offset + itmp_short_2d(i,j) * scalef
            end if
         end do
      end do
      deallocate(itmp_short_2d)
   ...
   ! get solar zenith angle (SOZ)
   nf_status = nf_INQ_VARID(ncid, 'SOZ', varid)
   if (nf_status /= NF_NOERR) then
     print *, 'Error: Could not find variable SOZ'
     stop 1
   end if
   allocate(itmp_short_2d(nx, ny))
   nf_status = nf_GET_VARA_INT2(ncid, varid, istart(1:2), icount(1:2), itmp_short_2d(:,:))
   ...
   do j = 1, ny
      do i = 1, nx
         if ( itmp_short_2d(i,j) /= ifill ) then
            gsolzen(i,j) = offset + itmp_short_2d(i,j) * scalef
         end if
      end do
   end do
   deallocate(itmp_short_2d)

   ! get satellite zenith angle (SAZ)
   nf_status = nf_INQ_VARID(ncid, 'SAZ', varid)
   if (nf_status /= NF_NOERR) then
     print *, 'Error: Could not find variable SAZ'
     stop 1
   end if
   allocate(itmp_short_2d(nx, ny))
   nf_status = nf_GET_VARA_INT2(ncid, varid, istart(1:2), icount(1:2), itmp_short_2d(:,:))
   ...
   do j = 1, ny
      do i = 1, nx
         if ( itmp_short_2d(i,j) /= ifill ) then
            gsatzen(i,j) = offset + itmp_short_2d(i,j) * scalef
         end if
      end do
   end do
   deallocate(itmp_short_2d)

end subroutine read_HS_NC


subroutine read_HS_HSD(data_dir, hsd_fnames, satid, nsegm, jday, longitude, latitude, brit, solzen, satzen, valid)

   implicit none
   character(len=256), intent(in) :: data_dir
   character(len=256), intent(in) :: hsd_fnames(:)
   character(len=3),   intent(in) :: satid
   integer(i_kind),    intent(in) :: nsegm
...
subroutine decode_himawari_name(fname, finfo, iband, nsegm, satid, file_time, jday, is_CLP, is_BCM, is_Phase, is_HT, is_HS_HSD, is_HS_NC)
   ...
   integer(i_kind),   intent(out) :: nsegm
   ...
   logical,           intent(out) :: is_HS_HSD
   logical,           intent(out) :: is_HS_NC

   character(len=4)  :: region     ! 'FLDK'
   character(len=3)  :: resolution ! 'R20, R21, R05, etc'
   ...
   integer(i_kind)   :: year, month, day, hour, minute, sec, sec1, sec2, isegm
   ...
   !BT HSD:     HS_H08_20210123_0000_B07_FLDK_R20_S0110.DAT
   !BT NC:      NC_H08_20180415_0000_R21_FLDK.02401_02401.nc

   nsegm = -99
   if ( fname(1:2) == 'HS' ) then
      is_HS_HSD = .true.
   else if ( fname(1:2) == 'NC' .and. fname(22:26) /= CLP_id ) then
      is_HS_NC = .true.
      iband  = -99
   else if ( fname(22:26) == CLP_id ) then
      is_CLP = .true.
      iband  = -99
   ...
   if ( .not. ( is_CLP .or. is_BCM .or. is_Phase .or. is_HT ) ) then
      if ( is_HS_HSD ) then
         ...
         jday = jday + day
      else if ( is_HS_NC ) then
         !NC_H08_20180418_0000_R21_FLDK.02401_02401.nc
         read(fname(1:2),   '(a2)')   finfo
         read(fname(4:6),   '(a3)')   satid
         read(fname(8:11),  '(i4)')   year
         read(fname(8:11),  '(a4)')   syear
         ...
         read(fname(19:20), '(i2)')   minute
         read(fname(19:20), '(a2)')   sminute
         read(fname(22:24), '(a3)')   resolution
         read(fname(26:29), '(a4)')   region
         read(fname(31:35), '(a5)')   pixelnumber
         read(fname(37:41), '(a5)')   linenumber
         ! 2017-10-01T18:02:00.0Z
         !print*, syear, smonth, sday, shour, sminute
         file_time = syear//'-'//smonth//'-'//sday//'T'//shour//':'//sminute//':00.0Z'
         ...
         write(0,*) 'Error decoding brightness temperature. Provide a valid file'
         stop
      end if
